...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    access_token = create_token(
        data={"sub": user.username}, expires_delta=ACCESS_TOKEN_EXPIRES_MINUTES
    )
    refresh_token = create_token(
        data={"sub": user.username},
        expires_delta=REFRESH_TOKEN_EXPIRES_MINUTES,
        refresh=True,
    )
    return {
...
async def delete_session(
    session_id: str, current_user: TokenData = Depends(current_user)
):
    try:
...
async def text_processing(
    session_id: str, text: str = Form(), current_user: TokenData = Depends(current_user)
):
    try:
        user = get_user(db, current_user.username)
        if session_id == "new":
